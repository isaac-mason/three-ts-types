...
const fnWithObjectArgs = tslFn(({ a, b }: { a: ShaderNodeObject<Node>; b: ShaderNodeObject<Node> }) => a.add(b));
assertSwizzable<Node>(fnWithObjectArgs({ a: color(0, 0.5, 0), b: color(1, 0.5, 0) }));

const fnWithArrayArgs = tslFn(([a, b]: [a: ShaderNodeObject<Node>, b: ShaderNodeObject<Node>]) => a.add(b));
assertSwizzable<Node>(fnWithArrayArgs(color(0, 0.5, 0), color(1, 0.5, 0)));
